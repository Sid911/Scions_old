/*
    Used under Apache License Version 2.0, January 2004 http://www.apache.org/licenses/
 */
#ifndef SCIONS_RESULT_H
#define SCIONS_RESULT_H
/*
...
namespace sc {
    ...
    struct Result {
        ...
        typedef sc::result::details::Storage <T, E> storage_type;

        Result(sc::result::types::Ok <T> ok)
                : ok_(true) {
            storage_.construct(std::move(ok));
        }

        Result(sc::result::types::Err <E> err)
                : ok_(false) {
        ...
        Result(Result &&other) {
            if (other.isOk()) {
                sc::result::details::Constructor<T, E>::move(std::move(other.storage_), storage_, sc::result::details::ok_tag());
                ok_ = true;
            } else {
                sc::result::details::Constructor<T, E>::move(std::move(other.storage_), storage_, sc::result::details::err_tag());
                ok_ = false;
            }
        }
        ...
        Result(const Result &other) {
            if (other.isOk()) {
                sc::result::details::Constructor<T, E>::copy(other.storage_, storage_, sc::result::details::ok_tag());
                ok_ = true;
            } else {
                sc::result::details::Constructor<T, E>::copy(other.storage_, storage_, sc::result::details::err_tag());
                ok_ = false;
            }
        }
        ...
        ~Result() {
            if (ok_)
                storage_.destroy(sc::result::details::ok_tag());
            else
                storage_.destroy(sc::result::details::err_tag());
        }
        ...
        template<typename Func,
                typename Ret =
                Result<
                        typename sc::result::details::ResultOkType<
                                typename sc::result::details::result_of<Func>::type
                        >::type,
                        E>
        >
        Ret map(Func func) const {
            return sc::result::details::map(*this, func);
        }

        template<typename Func,
                typename Ret =
                Result<T,
                        typename sc::result::details::ResultErrType<
                                typename sc::result::details::result_of<Func>::type
                        >::type
                >
        >
        Ret mapError(Func func) const {
            return sc::result::details::mapError(*this, func);
        }

        template<typename Func>
        Result<T, E> then(Func func) const {
            return sc::result::details::then(*this, func);
        }

        template<typename Func>
        Result<T, E> otherwise(Func func) const {
            return sc::result::details::otherwise(*this, func);
        }
        ...
        Ret orElse(Func func) const {
            return sc::result::details::orElse(*this, func);
        }
    ...
    bool operator==(const Result<T, E> &lhs, const Result<T, E> &rhs) {
        static_assert(sc::result::comparable::EqualityComparable<T>::value,
                      "T must be EqualityComparable for Result to be sc");
        static_assert(sc::result::comparable::EqualityComparable<E>::value,
                      "E must be EqualityComparable for Result to be sc");
    ...
    template<typename T, typename E>
    bool operator==(const Result<T, E> &lhs, sc::result::types::Ok <T> ok) {
        static_assert(sc::result::comparable::EqualityComparable<T>::value,
                      "T must be EqualityComparable for Result to be sc");
    ...
    template<typename E>
    bool operator==(const Result<void, E> &lhs, sc::result::types::Ok<void>) {
        return lhs.isOk();
    }

    template<typename T, typename E>
    bool operator==(const Result<T, E> &lhs, sc::result::types::Err <E> err) {
        static_assert(sc::result::comparable::EqualityComparable<E>::value,
                      "E must be EqualityComparable for Result to be sc");
