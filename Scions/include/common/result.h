#pragma once
/*
    Used under Apache License Version 2.0, January 2004
...
template <typename R> struct ResultErrType {
    typedef R type;
};
...
struct EqualityComparable<
    T,
    typename std::enable_if<true, typename details::void_t<
...
template <typename T, typename E> struct Result {
    ...
    explicit Result(sc::result::types::Ok<T> ok) : ok_(true) {
        storage_.construct(std::move(ok));
    }

    explicit Result(sc::result::types::Err<E> err) : ok_(false) {
        storage_.construct(std::move(err));
    }

    Result(Result &&other) noexcept {
        if (other.isOk()) {
            sc::result::details::Constructor<T, E>::move(
                std::move(other.storage_), storage_,
                sc::result::details::ok_tag());
            ok_ = true;
        } else {
            sc::result::details::Constructor<T, E>::move(
                std::move(other.storage_), storage_,
                sc::result::details::err_tag());
            ok_ = false;
        }
    }
    ...
    Result(const Result &other) {
        if (other.isOk()) {
            sc::result::details::Constructor<T, E>::copy(
                other.storage_, storage_, sc::result::details::ok_tag());
            ok_ = true;
        } else {
            sc::result::details::Constructor<T, E>::copy(
                other.storage_, storage_, sc::result::details::err_tag());
            ok_ = false;
        }
    }
    ...
    template <
        ...
        typename Ret = Result<
            typename sc::result::details::ResultOkType<
                typename sc::result::details::result_of<Func>::type>::type,
            E>>
    ...
    template <
        typename Func,
        typename Ret = Result<
            T, typename sc::result::details::ResultErrType<
                   typename sc::result::details::result_of<Func>::type>::type>>
